...
/*  if (req.query.search===undefined) {
  ...
  if(req.params.format==='.json'){
    if(req.query.search===undefined){
        models.Quiz.findAll({include: [models.Attachment]}, {where:["question like ?", "%"+req.query.search.replace(/\s/g,"%")+"%"], order: 'question ASC'})
          .then(function(quizzes){
            res.json({ quizzes: quizzes});
          }).catch(function(error){ next(error)});
    }
    else{
      models.Quiz.findAll({include: [models.Attachment]})({attributes: ['id', 'question', 'answer']})
      .then(function(quizzes){
        res.json({ quizzes: quizzes});
      })
      .catch(function(error){
        ...
        if(req.query.search===undefined){
          models.Quiz.findAll({include: [models.Attachment]}).then(
            function(quizzes) {
              res.render('quizzes/index.ejs', {quizzes: quizzes, errors: []});
            }
          ).catch(function(error){next(error)});
